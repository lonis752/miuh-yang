...
// - effect "fadeLeft": the whole frame fades in while sliding from the left
export default function ChapterImage({
  ...
  const isFadeLeft = effect === "fadeLeft";
  const y = useTransform(scrollYProgress, [0, 1], contain ? ["-4%", "4%"] : ["-7%", "7%"]);

  // Fade in from the left; skip the scroll-parallax so the transforms don't fight.
  const entrance = isFadeLeft
    ? {
  ...
  const img = (
    <motion.img
      style={isFadeLeft ? { scale: 1 } : { y, scale: contain ? 1 : 1.16 }}
  ...
  if (linkTo) {
    return (
      <MotionLink ref={ref} to={linkTo} className={`block ${frameBase}`} {...entrance}>
        {img}
  ...
  return (
    <motion.div ref={ref} className={frameBase} {...entrance}>
